"""Unbiased pass@k estimates for repeated agent trials."""
...
def estimate_pass_at_k(*, trials: int, successes: int, k: int) -> float:
    if trials <= 0:
...
def pass_at_k_from_outcomes(outcomes: Iterable[bool], k: int) -> float:
